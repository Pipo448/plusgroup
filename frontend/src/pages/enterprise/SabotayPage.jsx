...
// ============================================================
// PLUS GROUP — Jesyon Sabotay (Plan Antepriz sèlman)
// Vouch elektwonik, kredi telefòn, tès balans Sabotay
// 3 Lang: ht | fr | en
// ============================================================
...
import {
  ...
  AlertCircle, BarChart2, X, Wifi, WifiOff
} from 'lucide-react'
...
const T = {
  ht: {
    title: 'Sabotay',
    subtitle: 'Vant recharge ak vouch elektwonik Sabotay',
    newSale: 'Nouvo Vant',
    balance: 'Balans Sabotay',
    checkBalance: 'Verifye Balans',
    totalSales: 'Total Vant',
    totalProfit: 'Benefis Total',
    totalTransactions: 'Tranzaksyon',
    phone: 'Nimewo Telefòn',
    amount: 'Montan',
    operator: 'Operatè',
    sellingPrice: 'Pri Vant',
    costPrice: 'Pri Koute',
    profit: 'Benefis',
    status: 'Statut',
    pending: 'Annatant',
    ...
    failed: 'Echwe',
    cancel: 'Anile',
    confirm: 'Konfime Vant',
    noSales: 'Pa gen vant. Kòmanse premye vant ou a!',
    enterpriseOnly: 'Fonksyon sa a disponib sèlman pou Plan Antepriz.',
    enterPhone: 'Antre nimewo telefòn kliyan',
    selectOperator: 'Chwazi operatè',
    enterAmount: 'Antre montan recharge',
    enterSellPrice: 'Pri ou ap vann',
    enterCostPrice: 'Pri Sabotay achte',
    profitCalc: 'Kalkile Benefis Otomatik',
    today: "Jodi a",
    week: 'Semèn sa a',
    month: 'Mwa sa a',
    confirmSale: 'Konfime vant sa a?',
    apiConfig: 'Konfigirasyon API Sabotay',
    apiKey: 'Kle API Sabotay',
    saveConfig: 'Sove Konfigirasyon',
    testConnection: 'Tès Koneksyon',
    connected: 'Konekte',
  ...
  fr: {
    title: 'Sabotay',
    subtitle: 'Vente de recharges et vouchers électroniques Sabotay',
    newSale: 'Nouvelle Vente',
    balance: 'Solde Sabotay',
    ...
    enterPhone: 'Entrez le numéro du client',
    selectOperator: "Choisir l'opérateur",
    enterAmount: 'Montant de la recharge',
    enterSellPrice: 'Prix de vente',
    enterCostPrice: 'Prix Sabotay',
    profitCalc: 'Calcul automatique du bénéfice',
    today: "Aujourd'hui",
    week: 'Cette semaine',
    month: 'Ce mois',
    confirmSale: 'Confirmer cette vente?',
    apiConfig: 'Configuration API Sabotay',
    apiKey: 'Clé API Sabotay',
  ...
  en: {
    title: 'Sabotay',
    subtitle: 'Electronic recharge and voucher sales via Sabotay',
    newSale: 'New Sale',
    balance: 'Sabotay Balance',
    ...
    enterAmount: 'Recharge amount',
    enterSellPrice: 'Your selling price',
    enterCostPrice: 'Sabotay cost price',
    profitCalc: 'Auto profit calculation',
    today: 'Today',
    week: 'This week',
    month: 'This month',
    confirmSale: 'Confirm this sale?',
    apiConfig: 'Sabotay API Configuration',
    apiKey: 'Sabotay API Key',
...
const COLORS = {
  ...
  card: 'rgba(255,255,255,0.04)', border: 'rgba(201,168,76,0.2)',
  green: '#27ae60', red2: '#C0392B'
}
...
function SaleModal({ lang, onClose, onSave }) {
  ...
  const profitColor = profit === null ? '#94a3b8' : Number(profit) >= 0 ? COLORS.green : COLORS.red2
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div>
            ...
            <input value={form.phone} onChange={e => set('phone', e.target.value)}
              placeholder={t.enterPhone} type="tel"
          ...
          <div>
            ...
            <input value={form.amount} onChange={e => set('amount', e.target.value)}
              placeholder={t.enterAmount} type="number"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div>
              ...
              <input value={form.sellingPrice} onChange={e => set('sellingPrice', e.target.value)}
                placeholder={t.enterSellPrice} type="number"
            ...
            <div>
              ...
              <input value={form.costPrice} onChange={e => set('costPrice', e.target.value)}
                placeholder={t.enterCostPrice} type="number"
                style={{ width: '100%', padding: '8px 12px', borderRadius: 8, fontSize: 13, marginTop: 4,
          ...
          {profit !== null && (
            <div style={{
              ...
              background: Number(profit) >= 0 ? 'rgba(39,174,96,0.1)' : 'rgba(192,57,43,0.1)',
              border: `1px solid ${profitColor}30`
            }}>
              <span style={{ color: '#94a3b8', fontSize: 12 }}>{t.profit}: </span>
              <span style={{ color: profitColor, fontWeight: 800, fontSize: 18 }}>
                {Number(profit) >= 0 ? '+' : ''}{profit} HTG
              </span>
            </div>
          )}
        </div>
...
function ApiConfigModal({ lang, onClose }) {
  ...
  const handleSave = async () => {
    try {
      ...
      toast.error('Erè sove konfigirasyon')
    }
  }
      ...
      <div style={{
        ...
        {status && (
          <div style={{
            ...
            color: status === 'connected' ? COLORS.green : COLORS.red2
          }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handleTest} disabled={testing || !apiKey} style={{
            ...
            border: `1px solid ${COLORS.border}`, background: 'transparent', color: '#94a3b8'
          }}>{testing ? '...' : t.testConnection}</button>
...
const isEnterprise = ['antepriz', 'antrepriz', 'entreprise', 'enterprise']
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => api.get('/sabotay/sales', { params: { status: filter !== 'all' ? filter : undefined, period } })
      .then(r => r.data),
    onError: () => ({ sales: [], stats: {}, balance: null })
  })

  const { data: balanceData, refetch: refetchBalance } = useQuery({
    queryKey: ['sabotay-balance'],
    queryFn: () => api.get('/sabotay/balance').then(r => r.data),
    onError: () => ({ balance: null })
  })

  const sales = (data?.sales || []).filter(s =>
    !search || s.phone?.includes(search) || s.operator?.toLowerCase().includes(search.toLowerCase())
  )
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries(['sabotay-sales'])
      qc.invalidateQueries(['sabotay-balance'])
    ...
    <div style={{ padding: '24px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <button onClick={refetchBalance} style={{
          ...
          background: 'rgba(255,255,255,0.06)', color: '#94a3b8', cursor: 'pointer', fontSize: 12
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 20 }}>
        {[
          ...
          <div key={label} style={{
            ...
            <div style={{ color, fontWeight: 800, fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              {icon}{val}
            </div>
            ...
            return (
              <div key={sale.id} style={{
                ...
                <div style={{
                  ...
                  background: 'rgba(201,168,76,0.12)', color: COLORS.gold, minWidth: 64, textAlign: 'center'
                }}>{sale.operator}</div>
                ...
                <div style={{ flex: 1, minWidth: 120 }}>
                  ...
                  <div style={{ color: '#64748b', fontSize: 11 }}>
                    {new Date(sale.createdAt || Date.now()).toLocaleString('fr-FR')}
                  </div>
                </div>
                ...
                <div style={{ textAlign: 'right', minWidth: 90 }}>
                  <div style={{ color: '#fff', fontWeight: 800, fontSize: 16 }}>
                    {Number(sale.amount).toLocaleString('fr-HT')} HTG
                  </div>
                ...
                {profit !== null && (
                  <div style={{ textAlign: 'right', minWidth: 80 }}>
                    <div style={{ color: profit >= 0 ? COLORS.green : COLORS.red2, fontWeight: 700, fontSize: 14 }}>
                ...
                }}>{t[sale.status] || sale.status}</span>
              </div>
            )
          })}
        </div>
      ...
      {showSaleModal && (
        <SaleModal lang={lang} onClose={() => setShowSaleModal(false)}
