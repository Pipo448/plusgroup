...
import { TrendingUp, Package, Award, FileDown } from 'lucide-react'
...
const PERIOD_PRESETS = [
  { label: '7 jou', days: 7 },
  { label: '30 jou', days: 30 },
  { label: '90 jou', days: 90 },
]
  ...
  return (
    <div className="animate-fade-in">
      <div className="page-header">
        <h1 className="page-title">Rapò & Statistik</h1>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-slate-100 p-1 rounded-xl w-fit">
        {[
          { key:'sales', label:'Vant', icon:<TrendingUp size={15} /> },
          { key:'stock', label:'Stock', icon:<Package size={15} /> },
          { key:'top', label:'Top Pwodui', icon:<Award size={15} /> },
        ].map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === t.key ? 'bg-white shadow text-brand-700' : 'text-slate-500 hover:text-slate-700'}`}>
            {t.icon} {t.label}
          </button>
        ))}
      </div>

      {/* Date filter (pour ventes + top) */}
      {activeTab !== 'stock' && (
        <div className="flex items-center gap-3 mb-5">
          <div className="flex gap-1">
            {PERIOD_PRESETS.map(p => (
              <button key={p.days} onClick={() => setPreset(p.days)}
                className="btn-secondary btn-sm">{p.label}</button>
            ))}
          </div>
      ...
      {activeTab === 'sales' && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label:'Total Vant', val:`${fmt(salesReport?.totals?._sum?.totalHtg)} HTG`, color:'text-brand-700 bg-brand-50' },
              { label:'Total USD', val:`${fmt(salesReport?.totals?._sum?.totalUsd)} USD`, color:'text-purple-700 bg-purple-50' },
              { label:'Nbr Facture', val:salesReport?.totals?._count || 0, color:'text-emerald-700 bg-emerald-50' },
              { label:'Peye', val:`${fmt(salesReport?.totals?._sum?.amountPaidHtg)} HTG`, color:'text-emerald-700 bg-emerald-50' },
            ].map((s,i) => (
          ...
          {salesReport?.byStatus && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              <div className="card p-5">
                <h3 className="section-title">Vant pa Statut</h3>
                  ...
                  {salesReport.byStatus.map((s,i) => (
                    <div key={i} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-slate-400">{s._count} facture</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="card p-5">
                <h3 className="section-title">Repartisyon</h3>
      ...
      {activeTab === 'stock' && stockReport && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label:'Total Pwodui', val:stockReport.totalProducts },
              { label:'Stock ba', val:stockReport.lowStock, warn: stockReport.lowStock > 0 },
              { label:'Rupture', val:stockReport.outOfStock, warn: stockReport.outOfStock > 0 },
              { label:'Valè Stock (HTG)', val:`${fmt(stockReport.stockValue?.priceHtg)}` },
            ].map((s,i) => (
          ...
          <div className="table-container">
            <table className="table">
              <thead><tr><th>Pwodui</th><th>Kategori</th><th className="text-right">Kantite</th><th className="text-right">Sèy Alèt</th><th className="text-right">Val. Stock (HTG)</th></tr></thead>
              <tbody>
      ...
      {activeTab === 'top' && (
        <div className="table-container">
          <table className="table">
            <thead><tr><th>#</th><th>Pwodui</th><th className="text-center">Qte Vann</th><th className="text-right">Total HTG</th><th className="text-center">Nbi Komand</th></tr></thead>
            <tbody>
              ...
              )) || <tr><td colSpan={5}><div className="empty-state py-8"><p className="text-slate-400">Okenn done</p></div></td></tr>}
            </tbody>
          </table>
        </div>
      )}
