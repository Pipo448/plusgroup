...
const AdjustModal = ({ onClose }) => {
  ...
  // ✅ FIX: yon sèl mutation k ap itilize stockAPI.addMovement (ki egziste nan api.js)
  const mutation = useMutation({
    ...
    // onError: interceptor nan api.js ap deja montre erè a automatikman
  })
  ...
  const handleConfirm = () => {
    ...
    // ✅ FIX: map type frontend → type backend kòrèkteman
    const typeMap = {
      add:      'add',
      remove:   'remove',
      purchase: 'purchase',
      ...
      <div style={{
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: S.blueDim, border: `1.5px solid ${S.blue}40`, borderRadius: 12 }}>
                  ...
                  <button
                    ...
                    style={{ background: `${S.blue}20`, border: 'none', borderRadius: 8, width: 28, height: 28, cursor: 'pointer', color: S.blue, fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                  >✕</button>
                </div>
              ) : (
                <>
          ...
          <div style={{ display: 'flex', gap: 10, paddingTop: 4, borderTop: `1px solid ${S.border}` }}>
            <button
              onClick={onClose}
              style={{ flex: 1, padding: '11px', borderRadius: 10, background: '#F4F6FF', border: `1px solid ${S.border}`, color: S.muted, fontWeight: 700, cursor: 'pointer', fontSize: 14 }}
            >
              {t('common.cancel')}
            </button>
            <button
              ...
              style={{
                ...
                background: mutation.isLoading
                  ? `${S.orange}80`
                  : `linear-gradient(135deg, ${S.orange}, ${S.orangeLt})`,
