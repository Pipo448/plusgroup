    ...
    <div style={{ display:'flex', flexDirection:'column', gap:20, fontFamily:'DM Sans,sans-serif' }}>
      ...
      <div style={{
        ...
        <div style={{ position:'relative', zIndex:1, display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
          <StatCard label="Vant 30 jou"  val={`${fmt(totalVentes)} HTG`} icon={<TrendingUp size={15}/>}   color={D.gold}    sub="Total vant"/>
          <StatCard label="Peye"         val={`${fmt(totalPaye)} HTG`}   icon={<CheckCircle2 size={15}/>} color="#34d399"   sub={`${dashboard?.totalPaid?._count||0} fakti`}/>
          <StatCard label="Balans"       val={`${fmt(totalImpaye)} HTG`} icon={<Clock size={15}/>}         color={D.redLt}   sub={`${dashboard?.totalUnpaid?._count||0} impaye`}/>
          <StatCard label="Pasyal"       val={`${fmt(totalPasyal)} HTG`} icon={<Receipt size={15}/>}       color="#93c5fd"   sub={`${dashboard?.totalPartial?._count||0} dokiman`}/>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:16 }}>
        ...
        <div style={{ background:D.white, borderRadius:20, padding:'20px 20px 14px', boxShadow:D.shadow, border:`1px solid ${D.border}` }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18 }}>
            ...
            <Link to="/reports" style={{
              ...
              textDecoration:'none', padding:'5px 12px', borderRadius:8, background:D.blueDim, border:`1px solid ${D.border}`,
        ...
        <div style={{ background:D.white, borderRadius:20, padding:'20px', boxShadow:D.shadow, border:`1px solid ${D.border}` }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
            ...
            <Link to="/products" style={{ fontSize:11, fontWeight:700, color:D.red, textDecoration:'none',
            ...
            : <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                {lowStock.slice(0,5).map(p => (
                  <div key={p.id} style={{
                    ...
                    borderRadius:12, background:'rgba(192,57,43,0.05)', border:`1px solid ${D.border}`, transition:'all 0.15s'
                  }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14 }}>
        ...
        <KpiCard label="Peman Pasyal"   value={`${fmt(totalPasyal)} HTG`} count={`${dashboard?.totalPartial?._count||0} dokiman`}   icon={<Clock size={20}/>}          color={D.gold}    bg={D.goldDim}                        link="/invoices?status=partial"/>
        ...
        <div style={{
          ...
          <div style={{ display:'flex', gap:8 }}>
            <Link to="/quotes/new" style={{
              ...
              boxShadow:`0 3px 12px ${D.blue}40`, transition:'all 0.2s'
            }}
...
function InvoiceRow({ inv, idx }) {
  const [hov, setHov] = useState(false)
  const D = { blue:'#1B2A8F', blueLt:'#2D3FBF', blueDim:'rgba(27,42,143,0.07)', text:'#0F1A5C', muted:'#6B7AAB', border:'rgba(27,42,143,0.10)', gold:'#C9A84C' }
  return (
    <tr onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <td style={{ padding:'12px 16px', textAlign:'center' }}>
        {(() => {
          const map = { unpaid:{bg:'rgba(192,57,43,0.08)',color:'#C0392B',label:'Impaye'}, partial:{bg:'rgba(217,119,6,0.10)',color:'#D97706',label:'Pasyal'}, paid:{bg:'rgba(5,150,105,0.08)',color:'#059669',label:'Peye'}, cancelled:{bg:'rgba(100,100,100,0.08)',color:'#666',label:'Anile'} }
