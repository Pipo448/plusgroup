...
const getHeaders = () => {
  // ✅ Li done yo kòrèkteman nan Zustand persist store (pg-auth)
  const auth   = JSON.parse(localStorage.getItem('pg-auth') || '{}')
  const token  = auth?.state?.token
  const tenant = auth?.state?.tenant?.slug
  const branch = localStorage.getItem('plusgroup-branch-id')
  return {
