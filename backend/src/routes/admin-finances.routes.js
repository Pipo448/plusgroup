...
router.get('/summary', async (req, res) => {
  try {
    ...
    const [depansR, depansPrevwaR, revniR, achaR, vantR, depoSolR, pemanSolR, depansJodiR, revniJodiR, depansParKatR, dernye10R, kapitalR] = await Promise.all([
      ...
      prisma.$queryRawUnsafe(`SELECT COALESCE(SUM(amount),0) as total, COUNT(*) as count FROM admin_finances WHERE tenant_id='${tenantId}' AND type='depo_sol' AND status='active'`),
      prisma.$queryRawUnsafe(`SELECT COALESCE(SUM(amount),0) as total, COUNT(*) as count FROM admin_finances WHERE tenant_id='${tenantId}' AND type='peman_sol' AND status='active'`),
